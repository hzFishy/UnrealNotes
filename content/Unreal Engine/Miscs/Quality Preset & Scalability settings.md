You can see what the engine does when you chose between `Scalable` and `Maximum` in the function `FHardwareTargetingModule::GatherSettings`. I don't know if its the only place where the engine edit's the `.ini` files using the selected Quality Preset.

Showing here only the differences between the two `DefaultEngine.ini` when creating a blank project in 5.4.4.
