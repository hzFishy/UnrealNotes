...
# AI Possession
Inside `APawn::PostInitializeComponents` depending on the value of `AutoPossessAI` `APawn::SpawnDefaultController` will be called which creates a `NewController` and call `AController::Possess` on it.
...
# AI focus/rotation
**Rotation**<br>
Using `SetControlRotation` on AIController wont do anything (if on the pawn you didn't explicit ask for the possessed pawn to use the controller desired rotation), you must set the actor rotation on possesed pawn.
